...
def TSVoutput(output_data):
	output_tsv=open('kishouchou_Alldata_2013Tokyo.txt', 'w') ######################作成するファイル名。ファイル名が同じだとこのプログラムを実行する度に上書きされていくため、複数地域についてデータ取得する場合、ファイル名を毎回変えて実行する##############
	print output_tsv	
...
def autoread_html(): #気象庁のデータベースから全データを検索して weatherinfo_list に格納する
	...
	for i in range(1,13):
		...
		url='http://www.data.jma.go.jp/obd/stats/etrn/view/daily_s1.php?prec_no=44&block_no=47662&year=2012&month=0' + month +'&day=&view=p1' ##############他の地域や年についてデータを取得したい場合ここをいじる#################
